...
/* Two-step verification: the switch, and what it opens.
 *
 * Kept out of the sign-in page because this is a thing you turn on while
 * already signed in — the sign-in page only ever *asks* for a code.
 */
(function () {
  var row = document.getElementById("two-step-row");
  if (!row) { return; }

  var sheet = document.getElementById("two-step-sheet");
  var state = document.getElementById("two-step-state");
  ...
  $$("two-step-close").addEventListener("click", function () { sheet.hidden = true; });
  $$("two-step-scrim").addEventListener("click", function () { sheet.hidden = true; });
